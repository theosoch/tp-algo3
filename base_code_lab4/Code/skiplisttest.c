...
SkipList buildlist(int num) {
    ...
    if (input!=NULL) {
        ...
        if(fscanf(input, "%u", &level) == EOF) exit(EXIT_FAILURE);
        d = skiplist_create(level);
        
        if(fscanf(input, "%u", &nb_values) == EOF) exit(EXIT_FAILURE);
        for (unsigned int i=0;i< nb_values; ++i) {
            if(fscanf(input, "%d", &value) == EOF) exit(EXIT_FAILURE);
...
/**
 * @brief Prints the content of a given skiplist in the given out-stream.
 * 
 * @param l 
 */
void fprint_skiplist(FILE* outstream, SkipList l, unsigned char iterator_direction) {
    int l_size = skiplist_size(l);

    fprintf(outstream, "Skiplist (%d)\n", l_size);

    SkipListIterator it = skiplist_iterator_create(l, iterator_direction);
    ...
    for(
        ...
        fprintf(outstream, "%d ", skiplist_iterator_value(it));
    }
...
/**
 * @brief Prints content of a given skiplist in the stdout.
 * 
 * @param l 
 */
void print_skiplist(SkipList l, unsigned char iterator_direction) {
    fprint_skiplist(stdout, l, iterator_direction);
}
...
void test_construction(int num) {
    ...
    print_skiplist(l, FORWARD_ITERATOR);
...
void test_search(int num) {
    ...
    char *file = gettestfilename("search", num);
    FILE* fileinput = fopen(file, "r");

    unsigned int value_count = 0;
    int value = 0;

    unsigned int min_nb_operations = 0;
    unsigned int max_nb_operations = 0;
    unsigned int average_nb_operations = 0;
    ...
    if (fileinput!=NULL) {
        ...
        if(fscanf(fileinput, "%u", &value_count) == EOF) exit(EXIT_FAILURE);

        for (unsigned int i=0; i < value_count; ++i) {
            if(fscanf(fileinput, "%d", &value) == EOF) exit(EXIT_FAILURE);

            // 

            unsigned int nb_operations = 0;

            bool found = skiplist_search(l, value, &nb_operations);

            if(nb_operations < min_nb_operations || min_nb_operations == 0) min_nb_operations = nb_operations;
            if(nb_operations > max_nb_operations) max_nb_operations = nb_operations;
            average_nb_operations += nb_operations;
    ...
    average_nb_operations = average_nb_operations / value_count;
...
void test_search_iterator(int num) {
    ...
    if (fileinput!=NULL) {
        ...
        for (unsigned int i=0; i < value_count; ++i) {
            ...
            for(
                it = skiplist_iterator_begin(it);
                !skiplist_iterator_end(it) && !found;
                it = skiplist_iterator_next(it)
            ) {
                if(skiplist_iterator_value(it) == value) found = true;
                nb_operations++;
            }
 ...
 */
void test_remove(int num) {SkipList l = buildlist(num);

    // 

    char *file = gettestfilename("remove", num);
    FILE* fileinput = fopen(file, "r");
    ...
    print_skiplist(l, BACKWARD_ITERATOR);
